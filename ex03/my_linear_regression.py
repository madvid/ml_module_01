...
class MyLinearRegression():
        ...
        if (not isinstance(thetas, (np.ndarray, tuple, list))) \
            or (not isinstance(alpha, (int, float))) \
                or (not isinstance(max_iter, int)):
                    raise TypeError("At least one of the parameters is not of expected type.")
    ...
    def _gradient_(self, x, y):
        """ Private function gradient, there is no test perform on the parameters.
        It is to avoid to perform useless same tests as each call of gradient in fit
        method.
        """
